...
Updated by Jacqueline (daily) or manually. Last updated: 2026-04-04.

---

## TODAY -- 2026-04-04 (Saturday)
...
The Ed25519 private key generated by Ron on Apr 3 is stored ONLY in the LoreConvo session log.
Without it, Pro license keys cannot be generated for paying customers.

Retrieve with:
```
python scripts/save_to_loreconvo.py --search "license private key"
```
Save to password manager as "Labyrinth Analytics License Signing Key".
...
#### Save license signing private key (URGENT -- do this now)
The Ed25519 private key generated by Ron on Apr 3 is stored ONLY in the LoreConvo session log.
Retrieve: `python scripts/save_to_loreconvo.py --search "license private key"`
Save to password manager as "Labyrinth Analytics License Signing Key".
Without it, Pro license keys cannot be generated for paying customers.
Where tracked: `CLAUDE.md` Debbie TODO #4
...
#### Create GitHub repo + push marketplace (BEFORE doing this: fix email in marketplace.json)
- SEC-015: `marketplace.json` has personal Gmail in owner.email field. Replace with business
  email (e.g., hello@labyrinthanalyticsconsulting.com) or remove the field before making repo public.
- Then: Create `labyrinth-analytics/claude-plugins` on GitHub
