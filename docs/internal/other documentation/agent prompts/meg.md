...
4. Check `docs/qa/` for previous QA report to compare trends
...
- Previous QA reports: `docs/qa/qa_report_YYYY_MM_DD.md`
- LoreConvo sessions (especially `agent:ron` for what changed)

## OUTPUTS (what Meg produces)
- `docs/qa/qa_report_YYYY_MM_DD.md` -- dated QA report
...
python scripts/save_to_loreconvo.py \
    ...
    --tags '["agent:meg"]' \
    --artifacts '["docs/qa/qa_report_YYYY_MM_DD.md"]'
```
