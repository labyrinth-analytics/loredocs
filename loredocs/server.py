...
# ---------------------------------------------------------------------------
# Notion Import Bridge (SH-13222: PART:interfaces, SH-13221: PART:security)
# ---------------------------------------------------------------------------
...
from .notion_import import (
    ...
    VaultNotFoundError,
    VaultAmbiguousError,
    WorkspaceSaturationError,
    _notion_extra_available,
    _resolve_vault as _resolve_notion_vault,
    _validate_token_arg_consistency,
    _validate_notion_ids,
    _encode_continuation_token,
    _decode_continuation_token,
    _get_max_block_depth,
    CheckpointManager,
    resolve_notion_token,
)
from .notion_checkpoint import redact_secrets as _redact_notion_secrets

# Cached at module load -- checked once at server startup.
_NOTION_AVAILABLE, _NOTION_DIAG = _notion_extra_available()


class ImportNotionInput(BaseModel):
    model_config = ConfigDict(str_strip_whitespace=True)

    vault: str = Field(..., description="Vault ID or name to import into", min_length=1)
    page_ids: Optional[List[str]] = Field(default=None, description="Notion page UUIDs; null/omit = empty")
    database_ids: Optional[List[str]] = Field(default=None, description="Notion database UUIDs; null/omit = empty")
    tags: Optional[List[str]] = Field(default=None, description="Additional tags to apply")
    category: str = Field(default="reference", description="LoreDocs category for imported documents")
    checkpoint_file: Optional[str] = Field(default=None, description="Path for resume state; null = env/default")
    resume: bool = Field(default=False, description="Skip pages already in checkpoint")
    max_pages: int = Field(default=50, description="Max pages per call (cap: 200)")
...
@mcp.tool(
    title="Notion Import Setup",
    name="vault_import_notion_setup",
    annotations={"readOnlyHint": True, "destructiveHint": False, "idempotentHint": True, "openWorldHint": False}
)
def vault_import_notion_setup() -> dict:
    """
    Report Notion import readiness and how to enable it. Read-only.
    Does not install or modify any packages.
    """
    import sys as _sys
    ...
    if available:
        return {
            "status": "ready",
            "message": "vault_import_notion is enabled.",
        }
